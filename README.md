...
IRIS is a strongly typed functional language, and describes a novel computer architecture that diverges from the usual von Neumann-style RISC architecture in many important and interesting ways. It allows for many high-level language features, such as higher-order functions and polymorphism, while still remaining simple and efficient as a low-level assembly language. The core IRIS spec consists of just 26 isomorphisms, with extensions planned for floating-point and vector instructions. Ideally, IRIS will be well suited not only for implementation in software, but also in hardware as a fully reversible IRIS-based microprocessor.

The Iris VM is currently being written in Rust, and will have builds available for all of the core targets of the Rust toolchain. Eventually, we also plan to implement Iris on top of the seL4 microkernel platform as part of a reversible high-level language runtime and operating system.
...
- a tempspace memory block, for holding values temporarily
